...
sleep 30
LOCAL_IP=$(kubectl get pods -l app=red -o jsonpath='{.items[0].status.podIPs[].ip}')
REMOTE_IP=$(kubectl get pods -l app=blue -o jsonpath='{.items[0].status.podIPs[].ip}')
...
ip addr add dev gretap1 192.168.1.1/24
